...
const renderSettingsInput = (label, value, {id = null, help = null}) => {
    return createElementFromTemplate(settingsFieldTemplate({
        label, id, help,
...
const renderSettingsReset = () => {
    ...
    $settings.appendChild(createElementFromTemplate('<form method="post"><h2>Settings</h2></form>'));
    $container.appendChild($settings);
    return $settings.querySelector('form');
}

const renderSettingsFooter = () => {
    const $footer = createElementFromTemplate('<div class="settings__footer"></div>');
    const $save = createElementFromTemplate('<button class="button is-primary">Save</button>');
    const $cancel = createElementFromTemplate('<button class="button">Cancel</button>');
...
const renderSettings = () => {
    ...
    $settings.appendChild(renderSettingsInput('Refresh Rate', 60, {
        id: 'refresh_rate',
        help: 'Minutes between refresh calls.'
    }));
