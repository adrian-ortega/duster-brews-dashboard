 ...
 * @return {Promise<google.auth.OAuth2>}
 */
const attemptToAuthorize = (credentials, callback) =>
  new Promise((resolve) => {
...
module.exports = {
  ...
  authorize: () =>
    new Promise((resolve, reject) => {
      fs.readFile(
        `${STORAGE_PATH}/${CLIENT_CREDENTIALS_PATH}`,
        (err, content) => {
          ...
          if (err) {
            if (fileMissing) {
              ...
              log.debug(
                "The rest of the app cannot continue without this file."
              );
              log.debug(
                `Please provide one and save it at: \n\t${chalk.green(
                  err.path
                )}.\n`
              );
