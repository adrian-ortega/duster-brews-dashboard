...
const {
  PLAATO_SPREADSHEET_ID,
  PLAATO_SPREADSHEET_RANGE,
} = require("../../../config");
...
const plaatoGet = (token, pin, keg, key) =>
  axios
    .get(`${PLAATO_API_BASE_URI}/${token}/get/${pin}`)
    .then(({ data }) => {
      if (isArray(data)) {
...
const getKegs = async (requestTimestamp) => {
  try {
    ...
    const kegs =
      kegSheetData && kegSheetData.values
        ? await Promise.all(kegSheetData.values.filter(filter).map(transformer))
        : [];
    ...
    log.error("Error happened in getKegsFromGoogleSheets:", e);
    return [];
  }
};
